(function () {
  ...
  document.body.addEventListener("htmx:afterSwap", function () {
    ...
    var inDetailView =
      (mailDetail && mailDetail.style.display !== "none") ||
  ...
  function saveRecentCommand(cmdName) {
    ...
    recentCommands = recentCommands.filter(function (c) {
      return c !== cmdName;
    });
    // Add to front
  ...
  function detectActiveContext() {
    ...
    if (expandedPanel) {
      ...
      if (panelId.indexOf("issue") !== -1 || panelId.indexOf("work") !== -1)
        return "Work";
      if (panelId.indexOf("ready") !== -1) return "Work";
      if (panelId.indexOf("pr") !== -1 || panelId.indexOf("merge") !== -1)
        return "Status";
    }
    // Check detail views
    var mailDetail = document.getElementById("mail-detail");
  ...
  function getOptionsForType(argType) {
    ...
    switch (argType) {
      case "rigs":
        ...
        break;
      case "polecats":
        rawOptions = cachedOptions.polecats || [];
        break;
      case "convoys":
      ...
      case "messages":
        ...
        break;
      case "crew":
        rawOptions = cachedOptions.crew || [];
        break;
      case "escalations":
        rawOptions = cachedOptions.escalations || [];
        break;
      default:
        return [];
    }
  ...
  function renderResults() {
    ...
    if (pendingCommand) {
      var options = pendingCommand.argType
        ? getOptionsForType(pendingCommand.argType)
        : [];
      var argFields = parseArgsTemplate(pendingCommand.args);

      var formHtml =
        '<div class="command-args-prompt">' +
        '<div class="command-args-header">gt ' +
        escapeHtml(pendingCommand.name) +
        "</div>";
      ...
      for (var i = 0; i < argFields.length; i++) {
        ...
        var hasOptions =
          isFirstField && pendingCommand.argType && options.length > 0;
        var noOptions =
          isFirstField && pendingCommand.argType && options.length === 0;
        ...
        formHtml +=
          '<label class="command-field-label" for="' +
          fieldId +
          '">' +
        ...
        if (hasOptions) {
          ...
          formHtml +=
            '<select id="' +
            fieldId +
            '" class="command-field-select" data-flag="' +
            (field.flag || "") +
            '">';
          formHtml +=
            '<option value="">Select ' +
          ...
          for (var j = 0; j < options.length; j++) {
            ...
            var optClass = opt.disabled
              ? ' class="option-disabled"'
              : opt.running
                ? ' class="option-running"'
                : "";
            formHtml +=
              '<option value="' +
      ...
      if (argFields.length === 0 && pendingCommand.args) {
        formHtml += '<div class="command-field">';
        formHtml +=
          '<input type="text" id="arg-field-0" class="command-field-input" placeholder="' +
          escapeHtml(pendingCommand.args) +
          '">';
        formHtml += "</div>";
      }
    ...
    if (currentQuery) {
      ...
      for (var i = 0; i < visibleCommands.length; i++) {
        ...
        var argsHint = cmd.args
          ? ' <span class="command-args">' + escapeHtml(cmd.args) + "</span>"
          : "";
      ...
      for (var j = 0; j < visibleCommands.length; j++) {
        ...
        if (item._section) {
          ...
          html +=
            ...
            escapeHtml(item._section) +
            "</div>";
          continue;
        }
        var cls2 = "command-item" + (j === selectedIdx ? " selected" : "");
        var argsHint2 = item.args
          ? ' <span class="command-args">' + escapeHtml(item.args) + "</span>"
          : "";
        var icon = item._recent
          ? '<span class="command-recent-icon">&#8635;</span>'
          : "";
        html +=
          '<div class="' +
          cls2 +
          ...
          '" data-cmd-args="' +
          escapeHtml(item.args || "") +
          '">' +
          icon +
          '<span class="command-name">gt ' +
          escapeHtml(item.name) +
          argsHint2 +
          "</span>" +
          '<span class="command-desc">' +
  ...
  function runWithArgsFromForm(fieldCount) {
    ...
    if (pendingCommand) {
      var fullCmd =
        pendingCommand.name + (args.length ? " " + args.join(" ") : "");
      pendingCommand = null;
      runCommand(fullCmd);
    }
  }
  ...
  function filterCommands(query) {
    ...
    if (!query) {
      ...
      for (var ri = 0; ri < recentCommands.length; ri++) {
        var recentCmd = allCommands.find(function (c) {
          return c.name === recentCommands[ri];
        });
      ...
      var remaining = allCommands.filter(function (c) {
        return !shownNames[c.name];
      });
      ...
      visibleCommands = scored.map(function (item) {
        return item.cmd;
      });
    }
    selectedIdx = 0;
    ...
    while (
      ...
      visibleCommands[selectedIdx]._section
    ) {
      selectedIdx++;
    }
    renderResults();
  ...
  function selectCommand(cmdName, cmdArgs) {
    ...
    if (cmdArgs) {
      var cmd = allCommands.find(function (c) {
  ...
  function runCommand(cmdName) {
    ...
    var matchedCmd = allCommands.find(function (c) {
      return cmdName.indexOf(c.name) === 0;
    });
  ...
  function showToast(type, title, message) {
    ...
    var icon = type === "success" ? "OK" : type === "error" ? "X" : "i";
    toast.innerHTML =
      ...
      .then(function (data) {
        ...
        if (data.threads && data.threads.length > 0) {
          ...
          data.threads.forEach(function (thread) {
            ...
            threadEl.className =
              "mail-thread" +
              (thread.unread_count > 0 ? " mail-thread-unread" : "");

            var last = thread.last_message;
            var hasMultiple = thread.count > 1;
            var countBadge = hasMultiple
            ...
            headerEl.innerHTML =
              ...
              "</span>" +
              (hasMultiple
                ? '<span class="mail-thread-preview"> — ' +
                  escapeHtml(last.body ? last.body.substring(0, 60) : "") +
                  "</span>"
                : "") +
              "</div>" +
              '<div class="mail-thread-right">' +
              '<span class="mail-time">' +
              formatMailTime(last.timestamp) +
              "</span>" +
              "</div>";
            ...
            if (hasMultiple) {
              ...
              thread.messages.forEach(function (msg) {
                ...
                msgEl.className =
                  "mail-thread-msg" + (msg.read ? "" : " mail-unread");
                ...
                msgEl.innerHTML =
                  ...
                  "</span>" +
                  '<span class="mail-time">' +
                  formatMailTime(msg.timestamp) +
                  "</span>" +
                  "</div>" +
                  '<div class="mail-thread-msg-subject">' +
  ...
  function formatMailTime(timestamp) {
    ...
    var dateStr =
      month + " " + day + yearPart + ", " + hours + ":" + minStr + " " + ampm;

    // Add relative time in parentheses for recent messages
    var relative = "";
    if (diff < 60000) relative = " (just now)";
    else if (diff < 3600000)
      relative = " (" + Math.floor(diff / 60000) + "m ago)";
    else if (diff < 86400000)
      relative = " (" + Math.floor(diff / 3600000) + "h ago)";
    else if (diff < 604800000)
      relative = " (" + Math.floor(diff / 86400000) + "d ago)";
        ...
        if (data.crew && data.crew.length > 0) {
          ...
          data.crew.forEach(function (member) {
            ...
            if (member.name === "mayor") {
              ...
              member.name === "witness" ||
              member.name.startsWith("witness-")
            ) {
              attachCmd = "gt witness attach";
            }

            tr.innerHTML =
              '<td><span class="crew-name">' +
              escapeHtml(member.name) +
              "</span></td>" +
              '<td><span class="crew-rig">' +
              escapeHtml(member.rig) +
              "</span></td>" +
              '<td><span class="' +
              stateClass +
              '">' +
              ...
              stateText +
              "</span></td>" +
              '<td><span class="crew-hook">' +
              (member.hook ? escapeHtml(member.hook) : "—") +
              "</span></td>" +
              '<td class="crew-activity">' +
              (member.last_active || "—") +
              "</td>" +
              "<td>" +
              sessionBadge +
              ...
              '" title="Copy attach command">Attach</button></td>';
      ...
      if (prevState && prevState !== newState) {
        if (newState === "finished") {
          showToast(
            "success",
            "Crew Finished",
            member.name + " finished their work!",
          );
          ...
          showToast(
            "info",
            "Needs Attention",
            member.name + " has questions for you",
          );
          playNotificationSound();
        }
      }
          ...
          if (completed + errors === beadIds.length) {
            if (errors > 0) {
              showToast(
                "error",
                "Partial",
                completed + " detached, " + errors + " failed",
              );
            } else {
      ...
      .then(function (data) {
        ...
        if (data.items && data.items.length > 0) {
          ...
          data.items.forEach(function (item) {
            ...
            var priBadge = "";
            if (item.priority === 1)
              priBadge = '<span class="badge badge-red">P1</span>';
            else if (item.priority === 2)
              priBadge = '<span class="badge badge-orange">P2</span>';
            else if (item.priority === 3)
              priBadge = '<span class="badge badge-yellow">P3</span>';
            else priBadge = '<span class="badge badge-muted">P4</span>';

            var sourceClass =
              item.source === "town"
                ? "ready-source ready-source-town"
                : "ready-source";
            ...
            tr.innerHTML =
              ...
              priBadge +
              "</td>" +
              '<td><span class="ready-id">' +
              escapeHtml(item.id) +
              "</span></td>" +
              '<td><span class="ready-title">' +
              ...
              "</span></td>" +
              '<td><button class="sling-btn" data-bead-id="' +
              escapeHtml(item.id) +
              '" title="Sling to rig">Sling</button></td>';
            tbody.appendChild(tr);
          });
  ...
  function openConvoyDetail(convoyId) {
    ...
    document.getElementById("convoy-detail-title").textContent =
      "Convoy: " + convoyId;
    ...
    fetch("/api/run", {
      ...
      .then(function (data) {
        ...
        if (!data.success) {
          document.getElementById("convoy-issues-empty").style.display =
            "block";
          document
            .getElementById("convoy-issues-empty")
            .querySelector("p").textContent =
            data.error || "Failed to load convoy";
          return;
        }
        ...
        issues.forEach(function (issue) {
          ...
          if (
            ...
            statusBadge =
              ...
              escapeHtml(issue.status || "Unknown") +
              "</span>";
          }

          tr.innerHTML =
            '<td class="convoy-issue-status">' +
            statusBadge +
            "</td>" +
            '<td><span class="issue-id">' +
            escapeHtml(issue.id) +
            "</span></td>" +
      ...
      .catch(function (err) {
        ...
        document
          .getElementById("convoy-issues-empty")
          .querySelector("p").textContent = "Error: " + err.message;
      });
  }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        if (line.startsWith("Progress:")) {
          document.getElementById("convoy-detail-progress").textContent = line
            .replace("Progress:", "")
            .trim();
        }
        continue;
  ...
  document
    .getElementById("convoy-back-btn")
  ...
  document
    .getElementById("convoy-create-back-btn")
    .addEventListener("click", cancelConvoyCreate);
  document
    .getElementById("convoy-create-cancel-btn")
    .addEventListener("click", cancelConvoyCreate);
  ...
  document
    .getElementById("convoy-create-submit-btn")
    .addEventListener("click", function () {
      var name = document.getElementById("convoy-create-name").value.trim();
      var issuesStr = document
        .getElementById("convoy-create-issues")
        .value.trim();
  ...
  document
    .getElementById("convoy-add-issue-btn")
    .addEventListener("click", function () {
  ...
  // Cancel add issue
  document
    .getElementById("convoy-add-issue-cancel")
    .addEventListener("click", function () {
      document.getElementById("convoy-add-issue-form").style.display = "none";
    });
  ...
  document
    .getElementById("convoy-add-issue-submit")
    .addEventListener("click", submitAddIssueToConvoy);
  ...
  document.addEventListener("click", function (e) {
    ...
    if (threadHeader) {
      ...
      if (msgId) {
        ...
        var msgsEl = threadEl
          ? threadEl.querySelector(".mail-thread-messages")
          : null;
        if (msgsEl) {
  ...
  // Reply button
  document
    .getElementById("mail-reply-btn")
    .addEventListener("click", function () {
      var subject = document.getElementById("mail-detail-subject").textContent;
      var replySubject = subject.startsWith("Re: ")
        ? subject
        : "Re: " + subject;
  ...
  document
    .getElementById("compose-mail-btn")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("compose-back-btn")
    .addEventListener("click", function () {
      mailCompose.style.display = "none";
      if (currentMessageId) {
        mailDetail.style.display = "block";
  ...
  // Cancel compose
  document
    .getElementById("compose-cancel-btn")
    .addEventListener("click", function () {
      mailCompose.style.display = "none";
      mailList.style.display = "block";
      currentMessageId = null;
  ...
  function populateToDropdown(selectedValue) {
    ...
    return fetch("/api/options")
      ...
      .then(function (data) {
        ...
        var addedValues = selectedValue
          ? [selectedValue.replace(/\/$/, "").toLowerCase()]
          : [];
      ...
      .catch(function (err) {
        ...
        select.innerHTML =
          '<option value="">Failed to load recipients</option>';
        select.disabled = false;
      });
  }
  ...
  function openIssueDetail(issueId) {
    ...
    document.getElementById("issue-detail-blocks-section").style.display =
      "none";
    ...
    fetch("/api/issues/show?id=" + encodeURIComponent(issueId))
      ...
      .then(function (data) {
        ...
        document.getElementById("issue-detail-title-text").textContent =
          data.title || "(no title)";
        document.getElementById("issue-detail-description").textContent =
          data.description || data.raw_output || "(no description)";
        ...
        if (data.priority) {
          ...
          if (data.priority === "P1") priorityEl.classList.add("badge-red");
            ...
            priorityEl.classList.add("badge-orange");
          else if (data.priority === "P3")
            priorityEl.classList.add("badge-yellow");
        ...
        if (data.status) {
          ...
          statusEl.className =
            "issue-status " + data.status.toLowerCase().replace(" ", "_");
        }
        ...
        if (data.depends_on && data.depends_on.length > 0) {
          ...
          var depsHtml = data.depends_on
            .map(function (dep) {
              return (
                '<span class="issue-dep-item" data-issue-id="' +
                escapeHtml(dep) +
                '">→ ' +
                escapeHtml(dep) +
                "</span>"
      ...
      .catch(function (err) {
        document.getElementById("issue-detail-title-text").textContent =
          "Error";
        document.getElementById("issue-detail-description").textContent =
          "Failed to load issue: " + err.message;
      });
  }
  ...
  function renderIssueActions(issueId, data) {
    ...
    var priNum =
      currentPriority.length === 2 ? parseInt(currentPriority[1], 10) : 2;
    ...
    fetch("/api/pr/show?url=" + encodeURIComponent(prUrl))
      ...
      .then(function (data) {
        ...
        if (data.labels && data.labels.length > 0) {
          document.getElementById("pr-detail-labels-section").style.display =
            "block";
        ...
        if (data.checks && data.checks.length > 0) {
          document.getElementById("pr-detail-checks-section").style.display =
            "block";
          var checksHtml = data.checks
            .map(function (check) {
              var checkClass = "pr-check";
              if (check.toLowerCase().includes("success"))
                checkClass += " success";
              else if (check.toLowerCase().includes("failure"))
                checkClass += " failure";
              else if (
                check.toLowerCase().includes("pending") ||
                check.toLowerCase().includes("in_progress")
              )
                checkClass += " pending";
              return (
                '<span class="' +
  ...
  function openSlingDropdown(btn) {
    ...
    fetch("/api/options?type=rigs")
      ...
      .then(function (data) {
        ...
        var html =
          '<div class="sling-dropdown-header">Sling ' +
          escapeHtml(beadId) +
          " to:</div>";
  ...
  function showReassignPicker(btn, escalationId) {
    ...
    picker.innerHTML =
      '<select class="reassign-select"><option value="">Loading...</option></select>' +
    ...
    // Confirm reassign
    picker
      .querySelector(".esc-reassign-confirm")
      .addEventListener("click", function () {
          ...
          .then(function (data) {
            if (data.success) {
              showToast(
                "success",
                "Reassigned",
                "Escalation reassigned to " + agent,
              );
              var row = btn.closest(".escalation-row");
              if (row) {
    ...
    // Cancel
    picker
      .querySelector(".esc-reassign-cancel")
      .addEventListener("click", function () {
        picker.remove();
        window.pauseRefresh = false;
      });
  ...
  function initTimelineFilters() {
    ...
    function applyFilters() {
      ...
      entries.forEach(function (entry) {
        ...
        if (
          selectedRig !== "all" &&
          entry.getAttribute("data-rig") !== selectedRig
        ) {
          show = false;
        }
        if (
          selectedAgent !== "all" &&
          entry.getAttribute("data-agent") !== selectedAgent
        ) {
          show = false;
        }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (
      existingDetail &&
      existingDetail.classList.contains("convoy-detail-row")
    ) {
    ...
    document.querySelectorAll(".convoy-detail-row").forEach(function (r) {
      r.remove();
    });
    document
      .querySelectorAll(".convoy-row.convoy-expanded")
      .forEach(function (r) {
    ...
    detailCell.innerHTML =
      '<div class="tracked-issues"><div class="tracked-issues-loading">Loading tracked issues...</div></div>';
    ...
    fetch("/api/run", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        command: "convoy status " + convoyId + " --json",
      }),
    })
      .then(function (r) {
        return r.json();
      ...
      .then(function (data) {
        if (!data.success) {
          detailCell.innerHTML =
            '<div class="tracked-issues"><div class="tracked-issues-error">Failed to load: ' +
        ...
        try {
          ...
          detailCell.innerHTML =
            '<div class="tracked-issues"><div class="tracked-issues-error">Failed to parse response</div></div>';
        }
      })
      .catch(function (err) {
        detailCell.innerHTML =
          '<div class="tracked-issues"><div class="tracked-issues-error">Request failed: ' +
  ...
  function renderConvoyIssues(cell, data) {
    ...
    if (issues.length === 0) {
      cell.innerHTML =
        '<div class="tracked-issues"><div class="tracked-issues-empty">No tracked issues</div></div>';
      return;
    }
    ...
    html +=
      "<thead><tr><th>Status</th><th>ID</th><th>Title</th><th>Assignee</th><th>Progress</th></tr></thead>";
    html += "<tbody>";
    ...
    for (var i = 0; i < issues.length; i++) {
      ...
      if (issue.status === "closed") {
        progress = '<span class="convoy-progress-done">Done</span>';
      } else if (issue.worker) {
        var workerName = issue.worker.split("/").pop();
        progress =
          '<span class="convoy-progress-active">@' +
          escapeHtml(workerName) +
          "</span>";
        if (issue.worker_age) {
          progress +=
            ' <span class="convoy-progress-age">' +
            escapeHtml(issue.worker_age) +
            "</span>";
        }
      }

      html +=
        '<tr class="tracked-issue-row tracked-issue-' +
        ...
        escapeHtml(issue.id) +
        "</span></td>" +
        '<td class="tracked-issue-title">' +
        escapeHtml(issue.title) +
        "</td>" +
        '<td class="tracked-issue-assignee">' +
        escapeHtml(assignee) +
        "</td>" +
        '<td class="tracked-issue-progress">' +
        progress +
        "</td>" +
        "</tr>";
    }
    ...
    html +=
      '<div class="tracked-issues-progress-bar"><div class="tracked-issues-progress-fill" style="width: ' +
      pct +
      '%;"></div></div>';
    html +=
